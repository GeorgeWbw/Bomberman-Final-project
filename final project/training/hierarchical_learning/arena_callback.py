...
class MyCallbacks(DefaultCallbacks):
    ...
    def on_episode_end(self, worker: RolloutWorker, base_env: BaseEnv,
        ...
        for i in range(1,4):
            self.opponent_scores.append(episode.last_info_for(f'agent_{i}_high'))
    ...
    def on_train_result(self, trainer, result: dict, **kwargs):
        ...
        current_policy = trainer.get_policy('policy_kill').get_weights()
        if result["policy_reward_mean"]["policy_kill"] > 0 or len(self.policies) == 0:
        ...
        trainer.workers.foreach_worker(lambda w: self.copy_weights(current_policy if np.random.rand() > 0.2 else np.random.choice(self.policies), w.get_policy('policy_kill_opp')))
